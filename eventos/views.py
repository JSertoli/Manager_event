...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        draw.text((240, 651), f"{participante.username}", font=fonte_nome, fill=(0, 0, 0))
        draw.text((771, 782), f"{evento.nome}", font=fonte_info, fill=(0, 0, 0))
        draw.text((828, 849), f"{evento.carga_horaria} horas", font=fonte_info, fill=(0, 0, 0))
        output = BytesIO()
